Ext.define('Admin.view.fidusia.DOrderDetailController', {
    extend: 'Ext.app.ViewController',
    refs: [
        {
            ref: 'doklayananfidusiaGrid', 			
            selector: '#doklayananfidusiaGrid'
        }
    ],
    alias: 'controller.doklayananfidusia',
    init: function() {
        this.control({
            '#doklayananfidusiaGrid': {
            ...
            'doklayananfidusiatambah button[text=Tambah]':{
                click: this.tambahDoklayanan
            },
            'doklayananfidusiatambah button[text=Simpan]':{
    ...
    onAddClick: function(grid){
        var view=Ext.create('Admin.view.fidusia.DOrderDetailTambah',{create:true});
    ...
    editDoklayanan: function(grid,record){
        view = Ext.widget('doklayananfidusiatambah');
    ...
    tambahDoklayanan: function(button){
        ...
        storedoklayanan = Ext.getStore('doklayananfidusia');
    ...
    onDelete: function(){
        var grid=Ext.getCmp('doklayananfidusiaGrid');
        var selection = grid.getView().getSelectionModel().getSelection()[0];
        var storedoklayanan = Ext.getStore('doklayananfidusia');
        if (selection) {
    ...
    filterChange: function(field, newValue) {
        var gr = Ext.getStore('doklayananfidusia');
        if (newValue) {
