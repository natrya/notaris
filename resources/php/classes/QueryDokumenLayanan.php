...
class QueryDokumenLayanan
{
	...
	public function getResults(stdClass $params)
    {
        ...
        $query = "select count(iddokumen) as total from dokumen where idorder_detil = ".$kriteria.";";
        $query.= "SELECT d.iddokumen,d.idorder_detil,namalayanan(o.idlayanan) as namalayanan,d.nama,d.keterangan FROM dokumen d LEFT JOIN order_detil o on d.idorder_detil=o.idorder_detil where d.idorder_detil=".$kriteria." ORDER BY iddokumen ASC LIMIT ".$mulai.",".$banyak;
    ...
    {
        $_db = $this->__construct();
        if($stmt = $_db->prepare("INSERT INTO dokumen (idorder_detil,nama,keterangan) VALUES (?,?,?)")) {
	...
	{
		$_db = $this->__construct();
		if ($stmt = $_db->prepare("UPDATE dokumen SET keterangan=? WHERE idorder_detil=?")) {
	...
	public function destroyRecord(stdClass $params)
	{
		...
		if(is_numeric($iddokumen)) {
			if($stmt = $_db->prepare("DELETE FROM dokumen WHERE iddokumen = ? LIMIT 1")) {
